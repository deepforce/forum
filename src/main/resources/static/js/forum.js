        ...
        success: function (response) {
            if (response.code === 200) {
                ...
                if (response.code === 2003) {
                    ...
                    if (isAccepted) {
                        window.open("https://github.com/login/oauth/authorize?client_id=cd1b2839576d0971f83c&redirect_uri=" + document.location.origin + "/callback&scope=user&state=1");
...
function selectTag(e) {
    ...
    if (previous) {
        let previous_tags = previous.split(",");
        if (previous_tags.indexOf(value) == -1) {
